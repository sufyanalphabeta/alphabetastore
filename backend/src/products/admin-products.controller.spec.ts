...
describe('AdminProductsController', () => {
  ...
  it('delegates admin list, detail, review list, and summary', async () => {
    const products = { findAllAdmin: jest.fn().mockResolvedValue({ items: [] }), findOneAdmin: jest.fn().mockResolvedValue({ id: 'p1' }) };
    const review = { list: jest.fn().mockResolvedValue({ items: [] }), summary: jest.fn().mockResolvedValue({ total: 1 }) };
